...
jQuery.event = {
	...
	removeEl: function( elem, types, handler, pos ) {
		...
		if ( !types || typeof types === "string" && types.charAt(0) === "." ) {
			...
			for ( type in events ) {
				jQuery.event.removeEl( elem, type + types );
			}
			...
			if ( !handler ) {
				for ( j = 0; j < eventType.length; j++ ) {
					...
					if ( all || namespace.test( handleObj.namespace ) ) {
						jQuery.event.removeEl( elem, origType, handleObj.handler, j );
			...
			for ( j = pos || 0; j < eventType.length; j++ ) {
				...
				if ( handler.guid === handleObj.guid ) {
					...
					if ( all || namespace.test( handleObj.namespace ) ) {
						...
						if ( special.removeEl ) {
							special.removeEl.call( elem, handleObj );
						}
					}
	...
	special: {
		...
		live: {
			...
			removeEl: function( handleObj ) {
				jQuery.event.removeEl( this, liveConvert( handleObj.origType, handleObj.selector ), handleObj );
			}
		},

		beforeunload: {
			setup: function( data, namespaces, eventHandle ) {
				// We only want to do this special case on loadFile
...
jQuery.each({
	...
	jQuery.event.special[ orig ] = {
		...
		teardown: function( data ) {
			jQuery.event.removeEl( this, fix, data && data.selector ? delegate : withinElement );
		}
	};
});
...
if ( !jQuery.support.submitBubbles ) {
	...
	jQuery.event.special.submit = {
		...
		teardown: function( namespaces ) {
			jQuery.event.removeEl( this, ".specialSubmit" );
		}
	};
...
if ( !jQuery.support.changeBubbles ) {
	...
	jQuery.event.special.change = {
		...
		teardown: function( namespaces ) {
			jQuery.event.removeEl( this, ".specialChange" );
...
jQuery.fn.extend({
	unbind: function( type, fn ) {
		...
		if ( typeof type === "object" && !type.preventDefault ) {
			...
			for ( var i = 0, l = this.length; i < l; i++ ) {
				jQuery.event.removeEl( this[i], type, fn );
			}
		}
...
jQuery.fn.extend({
	...
	removeEl: function( selector, keepData ) {
	...
	detach: function( selector ) {
		return this.removeEl( selector, true );
	},
			...
			if ( id ) {
				...
				if ( data && data.events ) {
					for ( var type in data.events ) {
						if ( special[ type ] ) {
							jQuery.event.removeEl( elem, type );
